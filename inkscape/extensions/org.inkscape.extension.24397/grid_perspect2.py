...
class GridPerspective(inkex.Effect):
    def __init__(self):
    ...
    def perspective_intersection(self, left_theta, right_theta):
        ...
        r = (self.right_x - self.left_x) / \
            (sin(right_theta)/tan(left_theta)-cos(right_theta))
    ...
    def effect(self):
        ...
        self.ymax = self.svg.unittouu(
            str(self.options.height)+self.options.size_unit)
        self.xmax = self.svg.unittouu(
            str(self.options.width)+self.options.size_unit)
        self.horizon = self.svg.unittouu(
            str(self.options.horizon)+self.options.size_unit)
        self.left_x = self.svg.unittouu(
            str(self.options.left_x)+self.options.size_unit)
        self.right_x = self.svg.unittouu(
            str(self.options.right_x)+self.options.size_unit)
        ...
        left_dtheta = (left_theta_max - left_theta_min) / \
            float(self.options.p_divs)

        right_dtheta = (right_theta_max - right_theta_min) / \
            float(self.options.p_divs)
        ...
        intersection = self.perspective_intersection(
            left_mid_theta, right_mid_theta)
        if intersection > 0 and intersection < self.xmax:
            draw_SVG_line(intersection, 0,
                          intersection, self.ymax,
                          self.options.div_th,
